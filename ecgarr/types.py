...
class EcgTrainParams(BaseModel):
    ...
    samples_per_patient: Union[int, List[int]] = Field(1000, description='# train samples per patient')
    val_samples_per_patient: Union[int, List[int]] = Field(1000, description='# validation samples per patient')
...
class EcgDemoParams(BaseModel):
    ...
    frame_size: int = Field(1250, description='Frame size')
    # EVB arguments
    vid_pid: Optional[str] = Field('51966:16385', description='VID and PID of serial device formatted as `VID:PID` both values in base-10')
